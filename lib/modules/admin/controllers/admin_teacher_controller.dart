...
class AdminTeacherController extends GetxController {
  ...
  bool _initialTeachersDelayShown = false;
  ...
  Future<void> fetchTeachers({bool reset = true}) async {
    ...
    try {
      if (reset && !_initialTeachersDelayShown) {
        await Future.delayed(const Duration(milliseconds: 1500));
        _initialTeachersDelayShown = true;
      }
      ...
      Get.snackbar('Teachers', _formatApiError(e));
    } catch (_) {
      Get.snackbar('Teachers', 'Failed to load teachers.');
    } finally {
      if (reset) {
